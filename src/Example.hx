...
class Example {
	...
	static public function main() {
		...
		for (index in 0...ttfGlyphs.length) {
			...
			var glyphInfo:GlyphInfo = ttfGlyphs.getGlyphInfo(index);
			
			var canvas:js.html.CanvasElement = truetype.Glyph2Canvas.getGlyphCanvas(glyphInfo, scale, translateY, '#eee', true);
            js.Browser.document.body.appendChild(canvas);
			
			var svg:Xml = truetype.Glyph2SVG.getGlyphSvg(glyphInfo, scale, translateY);
